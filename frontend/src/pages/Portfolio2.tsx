  ...
  return (
    ...
    >
      <motion.div 
        ...
        exit={{ y: 50, opacity: 0 }}
        className={`${darkMode ? 'bg-black text-white' : 'bg-white text-black'} rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto transition-all duration-300 ease-in-out transform hover:scale-[1.02]`}
      >
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-2xl font-bold">{project.title}</h3>
            <button onClick={onClose} className="text-current hover:text-gray-500 transition-colors duration-300">
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {project.technologies.map((tech, index) => (
                <span key={index} className={`${darkMode ? 'bg-white text-black' : 'bg-black text-white'} px-2 py-1 rounded text-sm`}>
                  {tech}
                </span>
              ))}
            </div>
...
const Portfolio2: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-black text-white' : 'bg-white text-black'} transition-colors duration-300`} style={{ fontFamily: 'Times New Roman, serif' }}>
      {/* Navbar */}
      <nav className={`${darkMode ? 'bg-black' : 'bg-white'} shadow-lg sticky top-0 z-40 transition-colors duration-300`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex-shrink-0 flex items-center">
              ...
              />
              <span className="text-xl font-bold transition-colors duration-300">Roshan Patil</span>
            </div>
      ...
      <div className="py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            >
              ...
              <div className="flex justify-center md:justify-start space-x-4">
                ...
                <a href="https://linkedin.com" className="transition-colors duration-300">
                  <span className="sr-only">LinkedIn</span>
                  <FaLinkedin className="h-8 w-8 transition-transform duration-300 hover:scale-110" />
                </a>
                <a href="mailto:johndoe@example.com" className="transition-colors duration-300">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {skills.map((skill, index) => (
                <motion.div
                  ...
                  className={`${darkMode ? 'bg-white text-black' : 'bg-black text-white'} rounded-lg shadow p-4 text-center transition-all duration-300 hover:shadow-lg hover:scale-105`}
                >
                  {skill}
                </motion.div>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {projects.slice(0, visibleProjects).map((project, index) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.5, delay: index * 0.1 }}
                  className={`${darkMode ? 'bg-white text-black' : 'bg-black text-white'} rounded-lg shadow overflow-hidden transition-all duration-300 hover:shadow-lg hover:scale-105`}
                >
                  <div className="p-6">
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {project.technologies.map((tech, techIndex) => (
                          <span key={techIndex} className={`${darkMode ? 'bg-black text-white' : 'bg-white text-black'} px-2 py-1 rounded text-sm`}>
                            {tech}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <motion.button 
                        ...
                        className={`w-full sm:w-auto ${darkMode ? 'bg-black text-white hover:bg-gray-800' : 'bg-white text-black hover:bg-gray-200'} font-bold py-2 px-4 rounded transition-all duration-300 hover:shadow-lg`}
                      ...
                      <motion.a
                        ...
                        className={`w-full sm:w-auto ${darkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-black text-white hover:bg-gray-800'} text-center font-bold py-2 px-4 rounded transition-all duration-300 hover:shadow-lg flex items-center justify-center`}
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
            ...
            {visibleProjects < projects.length && (
              <div className="text-center mt-8">
                <motion.button
                  ...
                  className={`${darkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-black text-white hover:bg-gray-800'} font-bold py-2 px-6 rounded-full transition-all duration-300 hover:shadow-lg`}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
                  View More Projects
          ...
          >
            ...
            <div className={`${darkMode ? 'bg-white text-black' : 'bg-black text-white'} rounded-lg shadow p-6 transition-all duration-300 hover:shadow-lg`}>
              <p className="mb-4">
              ...
              <motion.a
                ...
                download
                className={`inline-flex items-center ${darkMode ? 'bg-black text-white hover:bg-gray-800' : 'bg-white text-black hover:bg-gray-200'} font-bold py-2 px-4 rounded transition-all duration-300 hover:shadow-lg`}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className={`${darkMode ? 'bg-white text-black' : 'bg-black text-white'} rounded-lg shadow p-6`}>
              <div className="mb-4">
                ...
                <input
                  ...
                  required
                  className={`w-full px-3 py-2 border rounded-md ${darkMode ? 'border-black text-black' : 'border-white text-white'} bg-transparent`}
                />
              </div>
              <div className="mb-4">
              ...
              <motion.button
                type="submit"
                className={`${darkMode ? 'bg-black text-white hover:bg-gray-800' : 'bg-white text-black hover:bg-gray-200'} font-bold py-2 px-4 rounded transition-all duration-300 hover:shadow-lg`}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                Send Message
